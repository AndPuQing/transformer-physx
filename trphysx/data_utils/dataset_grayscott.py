        ...
        for key in h5_file.keys():
            ...
            for i in range(
                ...
                self.position_ids.append(
                    paddle.arange(0, self.block_size, dtype=paddle.int64) + i
                )
...
class GrayscottPredictDataset(GrayscottDataset):
    ...
    def recover(self, x0, mb_size: int = 96):
        ...
        Args:
            x0 (paddle.Tensor): [B, config.n_embd] Time-series of embedded vectors
    ...
    def __getitem__(self, i) -> paddle.Tensor:
        return {
            "input": self.examples[i],
            "targets": self.states[i],
            "positions": self.position_ids[i],
        }
