...
class CylinderViz(Viz):
    ...
    def _createColorBarVertical(
        self, fig, ax0, c_min, c_max, label_format="{:02.2f}", cmap="viridis"
    ):
        ...
        cbar = mpl.colorbar.ColorbarBase(
            ax_cbar, cmap=plt.get_cmap(cmap), orientation="vertical", ticks=ticks
        )
    ...
    def plotPredictionVorticity(
        ...
        def xGrad(u, dx=1, padding=(1, 1, 1, 1)):
            WEIGHT_H = (
                paddle.to_tensor([[[[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]]]]) / 8.0,
            )
            ux = F.conv2d(
                F.pad(u, padding, mode="replicate"),
                WEIGHT_H.to(u.device),
                stride=1,
                padding=0,
                bias=None,
            ) / (dx)
        ...
        def yGrad(u, dy=1, padding=(1, 1, 1, 1)):
            WEIGHT_V = (
                paddle.Tensor([[[[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]]]]).transpose(
                    -1, -2
                )
                / 8.0
            )
        ...
        vortPred = xGrad(y_pred[:, 1].unsqueeze(1), dx=dx) - yGrad(
            y_pred[:, 0].unsqueeze(1), dy=dy
        )
        vortTarget = xGrad(y_target[:, 1].unsqueeze(1), dx=dx) - yGrad(
    ...
    def plotEmbeddingPrediction(
        ...
        for i, field in enumerate(["ux", "uy", "p"]):
            ...
            ax[2, i].imshow(
                y_error[i, :, :], extent=[-2, 14, -4, 4], cmap=cmap1, origin="lower"
            )
